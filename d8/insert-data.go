...
func main() {
	...
	for catName, prodList := range realProducts {
		...
		for i, prodName := range prodList {
			// Membuat format SKU unik yang rapi
			sku := fmt.Sprintf("SKU-%s-%d%d", faker.UUIDDigit()[:4], categoryID, i+1)
			price := float64((rand.Intn(50) + 1) * 15000)
			stock := rand.Intn(80) + 10

			// Query disesuaikan, kita tambahkan RETURNING id untuk melihat ID produk yang digenerate PK-nya
			var productID int
			queryProd := `INSERT INTO products (sku, category_id, name, price, stock) 
						  VALUES ($1, $2, $3, $4, $5) RETURNING id`

			err := db.QueryRow(queryProd, sku, categoryID, prodName, price, stock).Scan(&productID)
			if err != nil {
				log.Printf("  Gagal insert produk %s: %v\n", sku, err)
			} else {
				// Sekarang kita bisa menampilkan ID Produk (Primary Key) dan SKU (Unique) sekaligus
				fmt.Printf("  -> [ID: %d] Berhasil [%s]: %-40s | Rp %10.2f\n", productID, sku, prodName, price)
			}
		}
		fmt.Println("--------------------------------------------------------------------------------")
	}

	fmt.Println("Proses pengisian data baru selesai!")
}
